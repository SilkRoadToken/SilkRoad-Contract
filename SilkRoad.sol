...
contract DividendDistributor is IDividendDistributor {
    ...
    uint256 public minimumTokenBalanceForDividends = 21000 * (10**9); // user must hold 10 token
...
contract JackPot {
    ...
    address immutable BUSD = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    ...
    constructor () {
        ...
        winAmount = 100 * 10**18; // win amount 100 BUSD
        minBalance = 21000 * 10**9; // minimumhold 21000 SILKROAD
    }
    ...
    function _checkLottery(address recipient) internal returns (bool) {
        ...
        uint256 _pot = IBEP20(BUSD).balanceOf(address(this));
    ...
    function distributeLottery(address _winnerAddress, uint256 _balanceWinner) internal returns(bool) {
        if(_balanceWinner >= minBalance) {
            ...
            IBEP20(BUSD).transfer(_winnerAddress, winAmount);
...
contract SILKROAD is Ownable, IBEP20, SafeToken, LockToken, JackPot {
    using SafeMath for uint256;
    ...
    string constant _name = "Silk Road Token";
    string constant _symbol = "SILKROAD";
    ...
    function swapBnbToBUSD() internal {
        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = address(BUSD);
